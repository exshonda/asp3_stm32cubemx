...
/*
 *  kernel.hのターゲット依存部（STM32N6570-DK用）
 *
 ...
 *  割込み数
 *
 *  STM32N657 の IRQn_Type の最大値は 194（LTDC_UP_ERR_IRQn）．
 *  STM32Cube_FW_N6_V1.1.1/Drivers/CMSIS/Device/ST/STM32N6xx/Include/
 *  stm32n657xx.h:249 で確認（enum IRQn_Type の最後の要素．なお 192 は欠番）．
 *  ベクタテーブルの整列（VTOR 要件）はこの値から target_kernel.py が
 *  自動計算する（211エントリ×4=844 → 1024バイト境界）．
 */
#define TMAX_INTNO (194 + 16)
